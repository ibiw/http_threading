"""
    ...
    HTTP Header:
        Content-Length entity header indicates the size of the entity-body, in bytes.
        Range is used in the request, to ask for a particular range (or ranges) of bytes.
        Content-Range is used in the response, to indicate which bytes the server is giving you
"""
import threading
import time
import requests
...
class ContentRequest():
    ...
    def start(self):
        ...
        headers = (
            {'Range': 'Bytes={}-{}'.format(*item), 'Accept-Encoding': '*'} for item in offset_range
            )
        threads_list = []
